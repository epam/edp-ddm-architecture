...
Для надання доступу до зовнішніх систем, необхідно переконатися, що створено відповідну точку виходу з реєстру на Платформі -- *ServiceEntry*.

Service Entry створюється автоматично, при розгортанні регламенту реєстру за допомогою Jenkins-пайплайну публікацій -- `*MASTER-Build-registry-regulations-publication-pipeline*`.

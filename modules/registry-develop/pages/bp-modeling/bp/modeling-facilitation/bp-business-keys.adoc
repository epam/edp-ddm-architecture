...
ProcessInstance shipmentInstance = runtimeService
  ...
  .singleResult();
----
TIP: Аргумент `orderId` і є бізнес-ключем для пошуку умовного замовлення.
====
...
Розглянемо приклад налаштування бізнес-ключів у процесі внесення паспортних даних особи із використанням бізнес-ключів, що передаються у функції `submission()`.

...
[NOTE]
====
Делегат автоматично додасть системні заголовки при виконанні запита до SOAP-сервісу. Тому визначення заголовків у скрипті є опціональним.

_(Опціонально)_ Приклад формування заголовків запита:

[source,groovy]
----
def requestHeaders = [:]
requestHeaders['SOAPAction'] = 'testAction'
requestHeaders['Content-Type'] = 'text/xml;charset=UTF-8;'
----

Підставте відповідне значення для свого запита замість `'testAction'`.
====

...
Для аналітичних представлень створіть окремий файл createAnalyticsViews.xml з шаблону _link:{attachmentsdir}/study-project/task-6/xml-temp/createAnalyticsViews.xml[createAnalyticsViews.xml]_
...
xref:study-project/study-tasks/task-1-registry-db-modeling.adoc[Завдання 1]);

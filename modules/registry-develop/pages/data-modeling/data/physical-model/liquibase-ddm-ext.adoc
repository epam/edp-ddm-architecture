...
Крім того, умова пошуку містить вказівки щодо стовпців, які повинні бути повернені в результаті запиту -- *`request_by_search_param_id`*, `*name*` та `*search_param*`.
====
+
.HTTP-запит до ресурсу із query-параметрами offset та ліміт
====
[source,http]
----
https://registry-rest-api-mdtu-ddm-edp-cicd-platform-demo.apps.cicd2.mdtu-ddm.projects.epam.com/get-requests-by-name?offset=0&limit=10&name=string
----
====
+
.Відповідь від API
...
[
  {
    "katottg": "string",
    "requestByKatottgId": "3fa85f64-5717-4562-b3fc-2c963f66afa6",
    "name": "string"
  }
]
----
...
[%collapsible]
====
swagger::{attachmentsdir}/data-model/sc/swagger.yml[]
====

page ::
Тип пагінації, що повертає інформацію про поточну сторінку, кількість елементів на сторінці, загальну кількість елементів та загальну кількість сторінок.
+
...
[source,xml]
----
<changeSet author="registry owner" id="create SC get_requests_by_search-param_page">
    <ext:createSearchCondition name="get_requests_by_search-param_page" pagination="page">
        <ext:table name="request_by_search_param">
            <ext:column name="request_by_search-param_id"/>
            <ext:column name="name"/>
            <ext:column name="search-param" searchType="startsWith" />
        </ext:table>
    </ext:createSearchCondition>
</changeSet>
----

Цей Search Condition створює умову пошуку з назвою `*get_requests_by_search-param_page*`, яка дозволяє виконувати пошук запитів з таблиці `*request_by_search_param*` за допомогою параметра `*search_param*` з пагінацією на основі сторінок (атрибут `*pagination="page"*`).

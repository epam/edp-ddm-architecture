...
| `name` || так | plugin name, has to be `oidc`
| `config.client_id` || так | OIDC Client ID
| `config.client_secret` || так | OIDC Client secret
| `config.discovery` | https://.well-known/openid-configuration | ні | OIDC Discovery Endpoint (`/.well-known/openid-configuration`)
| `config.scope` | openid | ні| OAuth2 Token scope. To use OIDC it has to contains the `openid` scope
| `config.ssl_verify` | false | ні | Enable SSL verification to OIDC Provider
| `config.session_secret` | | ні | Additional parameter, which is used to encrypt the session cookie. Needs to be random
| `config.introspection_endpoint` | | ні | Token introspection endpoint
| `config.allow_token_auth` | false | ні | Allows access with Bearer token. If enabled introspection endpoint has to be set
| `config.timeout` | | ні | OIDC endpoint calls timeout
| `config.introspection_endpoint_auth_method` | client_secret_basic | ні | Token introspection auth method. resty-openidc supports `client_secret_(basic\|post)`
| `config.bearer_only` | no | ні | Only introspect tokens without redirecting
| `config.realm` | kong | ні | Realm used in WWW-Authenticate response header
| `config.logout_path` | /logout | ні | Absolute path used to logout from the OIDC RP
| `config.unauth_action` | deny | так | Action if user not authenticated:nil - redirect to auth, pass - do not redirect, deny - do not redirect but return an error instead
| `config.access_token_header_name` | X-Access-Token | так | Name for header with access token
| `config.bearer_access_token` | "no" | так | Use bearer access token
| `config.id_token_header_name` | X-ID-Token | так | Name for header with ID token
| `config.user_header_name` | X-Userinfo | так | Name for header with user info
| `config.session_opts` | | ні | JSON configuration string for [lua-resty-session](https://github.com/bungle/lua-resty-session)
| `config.enable_authorization` | true | ні | Enable user authorization based on JWT token issuer, scope and roles
| `config.issuers_allowed` | | ні | List of allowed access token issuers (iss claim) in the following format: issuers_allowed:\["https://domain/issuer1", "https://domain/issuer2"]
| `config.scopes_required` | nil | ні | The scopes required to be present in access token for successful authorization
| `config.roles_required` | nil | ні | The roles (roles_claim claim) required to be present in the access token (at least one from the list) for successful authorization
| `config.realm_roles_required` | nil | ні | The realm roles (realm_access.roles claim) required to be present in the access token (at least one from the list) for successful authorization
| `config.client_roles_required` | nil | ні | The realm roles (resource_access claim) required to be present in the access token (at least one from the list) for successful authorization
| `config.token_claim_header_value` | | ні | Access token claim name whose values will be set to the custom "token-claim" request header

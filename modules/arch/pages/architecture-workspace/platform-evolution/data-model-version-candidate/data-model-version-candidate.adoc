...
Процес створення та розгортання тимчасових схем БД відбувається за допомогою окремого `VersionCandidateRegistryDevDBSchemaManager`, що знаходиться в `ddm-rrm-data-model-management`.
...
- Виклик процесу reconciliation кожні N хвилин (раз на добу?). Параметр повинен задаватись на рівні конфігурації `registry-regulation-management` сервісу. Може бути заданий у відповідній `ConfigMap`.
...
Для доступу до тимчасових БД необхідно використовувати `DataSource` per тимчасова БД. Існуючий код, що забезпечує роботу з БД мастер версії використовує Spring bean типу DataSource.


Для перевикористання механізму зчитування структури БД в DataModelSnapshot file пропонується створити custom spring bean scope під назвою `version-candidate`.

==== Основна механіка і принципи роботи и spring bean custom scopes
...
xref:architecture/registry/administrative/regulation-management/admin-portal/data-model/admin-portal-data-model-json-schema-description.adoc[Опис структури DataModelSnapshot]
...
== Обробка та відображення помилок в процесі розгортання тимчасових схем БД
...
`GET /versions/candidates/{versionCandidateId}/tables/refresh` виклик RestAPI під час такої ситуації заповнить відповідним чином:

TODO: описати більш детально

http error code: 500

- response structure DetailedErrorResponse
- details: stacktrace details
...
** CREATE/DROP тимчасові БД
** ALTER * onto `dev` databases
** READ еталонної БД

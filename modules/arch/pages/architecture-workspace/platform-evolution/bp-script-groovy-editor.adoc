...
[NOTE]
xref:architecture/registry/administrative/regulation-management/admin-portal/code-editor/code-editor-language-server-protocol.adoc[Результати POC] для ознайомлення з LSP протоколом та веб-редактором коду MonacoEditor
...
image::architecture-workspace/platform-evolution/bpmnio-groovy-editor/groovy_TM.svg[]

=== Механізми протидії ризикам безпеки та відповідність вимогам безпеки

. Безпечний канал передачі інформації:
.. HTTPS
.. WSS
. Аутентифікація користувача має відбуватись при кожному запиті.
. Аутентифікація повинна відбуватись до моменту відкриття сокету.
. Під час виходу з системи ініційованого користувачем або при автоматичному закінченні терміну діх сесії будь-яка комунікація з веб сокетом повинна бути зупинена.
. Впровадження ліміту на максимальний розмір запиту на рівні 30 kb.
. Обмеження сокету та кількості запитів має бути налаштований окремо на /groovy ендпоінт.
...
. Обмеження ресурсів для сервісу ddm-language-server:
...
. Перевірити цільову версію language-server на наявність RCE/SSRV вразливостей при аналізі та розборі надісланого коду.
...
.. Веб сторінка з Монако редактором дає містити налаштовані заголовки Content Security Policy (CSP).
..  Веб сторінка з Монако редактором дає містити заголовок X-Content-Type-Options: nosniff 
. Вимоги до логування:

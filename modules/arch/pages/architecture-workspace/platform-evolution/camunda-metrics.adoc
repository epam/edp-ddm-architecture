  ...
  endpoints:
    web:
      exposure:
        include: 'prometheus'
  metrics:
    enable: # Вимкнути експорт стандартних метрик
      all: false
----
...
.Приклад Grafana-дашборди для кастомізації (xref:attachment$architecture-workspace/platform-evolution/camunda-metrics/camunda-metrics.json[Завантажити])
[%collapsible]
====
[source,json]
----
include::attachment$architecture-workspace/platform-evolution/camunda-metrics/camunda-metrics.json[]
----
====

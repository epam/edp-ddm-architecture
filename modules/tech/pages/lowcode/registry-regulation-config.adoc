...
- Налаштування переліку доменів, поштові адреси на яких є забороненими для використання у реєстрі
...
- Використання визначеної адміністратором поштової скриньки для повідомлень до служби підтримки, відправлених через кабінети користувачів
...
== Редизайн управління налаштуваннями реєстру

=== Поточний дизайн управління налаштуваннями

Наразі зафіксовано:

- Нецільове використання _global-vars/camunda-global-system-vars.yml_ для налаштування _supportEmail_ реєстру
...
----
include::partial$lowcode/registry-config-regulation-structure.puml[]
----
...
[TIP]
В рамках редизайну, необхідно відкорегувати JSON-схему для валідації _global-vars/camunda-global-system-vars.yml_ та видалити _supportEmail_.
...
- Валідація _settings.general.contacts.support.email_ значення згідно актуальному переліку в _settings.validation.blacklist.domains_
...
* Створити _settings/settings.yml_ файл з налаштуваннями реєстру, як зазначено вище:

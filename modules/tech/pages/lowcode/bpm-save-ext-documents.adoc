...
def sourceFileInputStream = fileResponse.body().byteStream();

def documentMetadata = save_digital_document_from_stream(sourceFileInputStream, "digital-document.ext") // Temporary save file to object storage

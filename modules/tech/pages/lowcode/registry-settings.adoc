...
- Дублювання налаштування _themeFile_ в регламенті та конфігурації реєстру
...
=== Налаштування аутентифікаторів кабінетів

[TIP]
В майбутньому розглянути можливість перевикористання _settings/settings.yml_ для конфігурування реєстрових аутентифікаторів, щоб уникнути дублювання налаштувань.

Необхідно розширити конфігурації _dsCitizenAuthenticator_ та _dsOfficerAuthenticator_ в _registry-configuration.git_ та _registry-regulations-publication-pipeline.git_ додатковим атрибутом _title_. Додатково реалізувати підтримку параметризації сторінок аутентифікації додатковим параметром _title_.

.Налаштування аутентифікатора громадян
image::lowcode/dso-citizen-authenticator.png[dso-citizen-authenticator, 300]

.Налаштування аутентифікатора посадових осіб
image::lowcode/dso-citizen-authenticator.png[dso-citizen-authenticator, 300]

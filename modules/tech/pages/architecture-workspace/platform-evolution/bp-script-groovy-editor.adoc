...
- MaxPayloadSize ліміт (Kong limit, https://docs.konghq.com/hub/kong-inc/request-size-limiting/[Onboard plugin?])
- Socket timeout (Java application limit, Javascript application limit)
- Max number of sockets per user (backend application limit)
- Rate limits (packets within single socket limit or on JSON-RPS level method call limit)
- proper CORS, not wildcard(Kong or java application)

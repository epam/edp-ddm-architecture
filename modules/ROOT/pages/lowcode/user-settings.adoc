...
* xref::lowcode/citizen-onboarding.adoc[Процес онбордингу]
* Кабінет людини/громадянина
* Бізнес-процеси регламенту

[NOTE]
 Детальніше про структуру налаштувань можна дізнатися за xref::datafactory/settings.adoc[посиланням]
...
** Result variable - змінна бізнес-процесу, в яку буде записано відповідь від сервісу налаштувань
** Payload - налаштування які потрібно зберегти згідно xref::datafactory/settings.adoc[контракту]
...
[NOTE]
 За замовчуванням відносна адреса сервісу налаштувань не буде змінюватись для різних інсталяцій
платформи
...
|_settings_(_assignee_('user-task-input'))
_settings_(_completer_('user-task-input'))
_settings_(_initiator_())
|===

[NOTE]
 _assignee()_, _completer()_ та _initiator()_ є іншими JUEL функціями, які повертають UserDto як результат
виконання
...
[NOTE]
Детальніше про контракт взаємодії можна дізнатися за xref::datafactory/settings.adoc[посиланням]

Методи по роботі з налаштуваннями доступні через Kong API Gateway та вимагають автентифікації
користувача

...
=== Структура файлів
Карта доступу до бізнес-процесів є частиною регламенту та зберігається у директорії з назвою *bp-auth*. Права доступу налаштовуються окремо для чиновників та громадян у відповідних файлах *officer.yml* та *citizen.yml*

[NOTE]
При моделюванні карти доступу можуть використатись як регламентні, так
і системні ролі. Детальніше про ролі можна ознайомитися xref:lowcode/auth.adoc[за посиланням]
...
бізнес-процесів регламенту (*bp-auth/officer.yml*, *bp-auth/citizen.yml*) пайплайн публікації
...
[plantuml]
----
include::partial$lowcode/publish-registry-authz.puml[]
----

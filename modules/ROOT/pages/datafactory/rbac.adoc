...
Запит та оновлення всіх колонок таблиці можливе через CRUD операції, часткове звернення або оновлення можливе через додавання до моделі тегів searchCondition та partialUpdate.
...
NOTE: Всі <<CRUD>> операції та <<Search Conditions>> також враховують роль користувача, перевіряючи наявний доступ у *_ddm_role_permission_* по зазначеній схемі
...
<changeSet id="roles" author="registry owner">
    ...
    <ext:rbac>
        ...
        <ext:role name="passport_officer" realm="officer_realm">
            <ext:table name="person">
        ...
        <ext:role name="birth_officer" realm="officer_realm">

...
<changeSet id="roles" author="registry owner">
    ...
    <rbac roleName="isAuthenticated">
        <read>
            <item table="person" column="first_name"/>
            <item table="person" column="last_name"/>
        </read>
    </rbac>
    ...
    <rbac roleName="officer">
        <read>
            <item table="person" column="first_name"/>
            <item table="person" column="last_name"/>
            <item table="person" column="passport"/>
        </read>
        <update>
            <item table="person" column="first_name"/>
            <item table="person" column="last_name"/>
...
У зазанчаеному прикладі такий SearchConditions буде мати права доступу denayAll оскільки в жодної ролі нема права читання поля `inn`
...
=== Читання даних
Для перевірки можливості того чи іншого запиту перед його виконанням викликається процедура в яку передається список полів які будуть викликані та перелік ролей користувача. Процедура повертає true або false. У разі true відбуваєтьс запит, а у разі false користувачу буде відправлена 403 помилка.

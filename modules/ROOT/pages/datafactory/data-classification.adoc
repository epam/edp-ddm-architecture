...
<createTable name="FOP" ext:historyFlag="true">
    ...
    <column name="rnokpp" type="TEXT" classify="Private"/>
    <column name="address" type="TEXT" classify="Private"/>
</createTable>
----
...
[source, sql]
----
CREATE TYPE classification_enum AS ENUM ('Private', 'Confidential');
----

.Тип для визначення поля як такого що містить класифіковані дані
[source, sql]
----
CREATE TYPE classification_type AS (
...
[source, sql]
----
CREATE TYPE access_role_type AS (
    data_column_name TEXT,
    access_role TEXT []
                                );
----
...
Створюємо в таблиці три додаткових службових поля, що являють собою масив об'єктів новостворених типів: *dсm_classification* та *dсm_access_role*. Поле *dсm_data_owner* є посиланням на таблицю суб'єктів. Для поля *dcm_classification* автоматично встановлюється значення по замовчуванню — перелік полів, що містять класифіковані дані, та відповідна категорія з <<XML шаблон для класифікації даних, XML шаблону>>. Решта службових полів отримує пустий масив в якості значення по замовчуванню.
...
CREATE TABLE FOP (
    ...
    dcm_owner_subject_id UUID,
    dcm_access_role access_role_type [] default '{ }',
    CONSTRAINT fk_fop_owner_subject_id
        FOREIGN KEY (dcm_owner_subject_id)
...
|===
^|fop_id ^|name ^|rnokpp ^|address ^|ddm_classification ^|ddm_data_owner ^|ddm_access_role

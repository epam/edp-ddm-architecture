...
* Risk Mitigation: Effective functional testing minimizes the risk of releasing a faulty or substandard product to end-users. By identifying and resolving issues before deployment, it helps mitigate potential financial, reputation, and legal risks.
...
The Scope of functional testing includes all registry and platform components of the Platform.

== Functional Testing Approach
=== Functional Testing Methodologies

The functional testing follows a combination of manual and automated testing approaches. It includes unit testing, integration testing, system testing, acceptance testing, regression testing and functional testing best practice.
...
Quality Gates consist of all types of described testing. The CI/CD process is structured into several stages:

* Stage Cluster: Newly developed functionality is deployed to the stage cluster, and initial testing is performed:
...
* Install Cluster: The compiled platform is deployed to a separate cluster where installation testing is carried out from the following stages:
...
* Update Cluster: After installation validation, a new cluster is deployed for checking platform updates from the previous version to the new one, including testing the updates through the following stages:

** Regression tests
** Integration tests
** System tests
** Visual tests
** Manual update test cases execution

* Pre-Production Cluster: Once the platform's ability for updates and installations is confirmed, the update is installed on the pre-production cluster, where testing includes:

** Manual User Acceptance Tests (UAT) execution
** Alpha/beta tests by registry development teams

* Production Cluster: After a year-long development cycle, the following testing is conducted in the production environment:

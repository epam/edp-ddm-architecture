...
When compiling the Installer during the xref:installation/platform-deployment/platform-deployment-overview.adoc[Platform deployment], it is required to configure the digital signature keys. After that, you can update key information when editing the Platform configuration in Control Plane.
...
. Log in to *Control Plane*, the administrative control panel of the platform and registers, using the previously received login and password.
...
. Open the [.underline]#Platform Management# menu.
...
. Go to the [.underline]#Key data# section and make further settings.
...
Two types of keys can be configured:
//Можна налаштувати два типи ключів:

* `file key`: stored on the user media in the `*.dat` format.
...
=== Configuring the key on the file media
//=== Налаштування ключа на файловому носії

. Select `Media type` -- `File media` (default).
...
. To add the new system key, press the kbd:[Select File] button. In the new window, navigate to the folder where the key file in the `*.dat` format is stored, select it and click kbd:[Open].
...
. The following step is to enter the name of the Accredited key certification center (AKCC) in the `Key issuing AKCC` field. The name of the key issuing AKCC you can find in the certificate of the key.
+
...
. Add the parameters of interaction with the compatible key certification centers (link:https://iit.com.ua/download/productfiles/CAs.json[CAs.json]).
...
+
[NOTE]
This block specifies a list of keys, including the old ones _(for example, when replacing keys)_, so that everything that was previously signed with the old key is considered verified (validated). That is, the list of allowed keys must contain the data history of all keys used in the system for signing.
...
. Finally, check the entered information and click kbd:[Confirm].
...
. Select `Media Type` -- `Hardware Media`.
...
IIT Hryada-301 is a hardware and software cryptomodule.
...
+
For example, you can find the name of your AKCC in the certificate of your key.
...
+
. At the next step, enter the `AKCC host` parameter.
...
[TIP]
====
As implemented in Ukraine, the default value is `255.255.255.255`.
...
[NOTE]
====
As a result of updating the key data on the Control Plane interface, a new request is created to update the `cluster-mgmt` configuration that xref: # confirm-changes [must be confirmed].
//У результаті оновлення даних про ключ на інтерфейсі Control Plane, створюється новий запит на оновлення конфігурації `cluster-mgmt`, який xref:#confirm-changes[необхідно підтвердити].
====
...
Platform keys are updated by making changes to the configuration of the Digital signature service .
...
* The administrator edits the platform or register digital signature keys in the interface of the Control Plane administrative panel.
...
Below is the diagram for updating platform and register keys and the configuration of the digital signature service.
...
=== Configuration parameters and secret creation
...
The path to engine for the platform keys looks as follows:
...
[NOTE]
====
* The administrative dashboard overwrites the data in Vault when updating the key data.
//* Адміністративна панель перезаписує дані у Vault при оновленні інформації про ключі.

* The pipeline recreates secrets when updating the key data, that is, is idempotent.
//* Пайплайн при оновленні даних про ключ, перестворює секрети, тобто є ідемпотентним.
====

The content of the secrets that the pipeline creates based on values.yaml: ::
//Зміст секретів, які створює пайплайн на основі values.yaml: ::
+
.Secretes for a file key
...
. In the new window, compare 2 versions of the changes, make sure that the data you entered is correct, and click kbd:[Confirm].
...
+
.Inputting data about a hardware key to the values.yaml configuration
...
. Wait while the code is assembled. This may take a few minutes.

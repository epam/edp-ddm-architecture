...
In this step, you can specify the registry's system name, which is used in various data exchange operations on the Platform. Registry name cannot be changed once the registry is created.
...
* *Registry name* is a required field.
...
You can describe the purpose of the registry or provide its official name in the *Description* field. This information may be useful for business or legal reasons.
...
You will also be able to add administrators by editing the registry soon after its deployment.
...
A registry template defines the configuration options and the allocated amount of resources, including *CPU*, *RAM*, the number of nodes in *MachineSets*, and so on.
+
CAUTION: This is a required field.
+
...
NOTE: This refers to the version of the component's branch in the Gerrit repository that contains the corresponding version of the registry template.
+
CAUTION: This is a required field.
+
...
CAUTION: The step is optional. You can skip it when creating a registry and come back to these settings when editing your registry.
...
* *External mail server* (`external-mail-server`) is a mail server outside the Platform (such as *gmail* and so on).
...
In this step, you can configure the digital signature keys and certificates your registry will use. Once the registry is created, you will be able to update these settings by editing your registry.
...
Data in the *Key info* section is required for initializing the `digital-signature-ops` crypto service and applying the system signature, or system digital seal. Without this information, the crypto service will not start.
...
. In the *Allowed keys list*, specify the keys whose signature are considered valid.
+
[NOTE]
====
...
When the registry is deployed, all of its services are configured using the default values for requests, limits, and environment variables.
...
TIP: For details on configuring the resources, see xref:registry-management/сontrol-plane-registry-resources.adoc[].
...
In this step, you can set your own DNS names and upload SSL certificates in `.pem` format for the Keycloak service and user portals.
...
In this step, you can restrict access to certain registry components, such as user portals and administrative components.
...
In this step, you can configure authentication for service providers (officers), as well as enable or disable self-registration.
...
In this step, you can set up a validation to check whether business users have an active entry in the Unified state register (EDR). This allows the system to establish a connection between the user's QES and their legal records in the state register. This is an important aspect of the system's security and reliability that helps to validate user data and confirm their identity.
...
[TIP]
====
* To learn about creating backups _automatically_ and restoring the registry environment from them, see xref:admin:backup-restore/backup-schedule-registry-components.adoc[].

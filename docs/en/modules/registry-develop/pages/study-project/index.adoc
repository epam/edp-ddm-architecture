...
. Model an analytical view.
//. Надати доступ до аналітичного представлення.
. Provide access to the analytical view.
...
The section covers test tasks for self-assessment after completing the training. The tasks go from simple to complex:
...
* xref:registry-develop:study-project/control-tasks/control-task-2.adoc[]: Deepen your practical skills in developing the business processes.

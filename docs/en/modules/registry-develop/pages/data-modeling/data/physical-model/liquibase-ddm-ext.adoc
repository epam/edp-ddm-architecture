...
[NOTE]
====
For the entity's `primaryKey`, we strongly recommend using the `UUID` data type, as demonstrated in the example: `type="UUID"`.
====

[WARNING]
====

Each `createTable` tag is required to have a `historyFlag` attribute with a `true` value as part of the registry regulations' verification.
...
TIP: For details on creating tables, see xref:registry-develop:data-modeling/data/physical-model/liquibase-changes-management-sys-ext.adoc#create-table-hst[Table modeling scheme and history maintenance function].

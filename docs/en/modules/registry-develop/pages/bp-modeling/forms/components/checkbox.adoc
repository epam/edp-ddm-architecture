...
CAUTION: Use Checkbox component from the list of _Latest_ components.
...
//Компонент *Checkbox* підтримує велику кількість опцій для застосування при взаємодії із формою процесу, які розподілені за вкладками.
 Checkbox component supports a large number of options for interaction with the process form, which are distributed across tabs.
...
These functions use JS insert.
xref:bp-modeling/forms/components/general/eval.adoc[Див.]

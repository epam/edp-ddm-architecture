...
CAUTION: Use DateTime component from the list of _Latest_ components.
...
* *`Disable specific dates or dates by range`*: allows disabling specific dates or dates by range.
...
* *`Hour Step Size`*: the number of hours to increment in the time picker tool, default is 1.
* *`Minute Step Size`*: the number of minutes to increment in the time picker tool, default is 1.
* *`12 Hour Time (AM/PM)`*: display time in 12-hour format with AM/PM.

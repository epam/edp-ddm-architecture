...
image::ROOT:giz-cicd/ddm-platform-structural-view.drawio.png[]
...
+
[TIP]
You can learn more about the container orchestration platform here: https://epam.github.io/edp-ddm-architecture/en/platform/1.9.6/arch/architecture/container-platform/container-platform.html[Container orchestration platform].
...
+
[TIP]
Dive deeper into the intricacies of the Platform resource state management component: https://epam.github.io/edp-ddm-architecture/en/platform/1.9.6/arch/architecture/platform-installer/overview.html[Component for managing the state of platform resources].
...
See https://epam.github.io/edp-ddm-architecture/en/platform/1.9.6/admin/installation/platform-deployment/platform-deployment-overview.html[Deploying the Platform on target environments].
____
...
+
*INIT-CHECK*: Checks essential parameters and verifies `digital-signature-ops` certificates (_initial deployment only_).
+
...
+
*INSTALL-ADDITIONAL-COMPONENTS*: Deploys `user-management` and uploads `digital-signature-ops` certificates.
+
...
TIP: For a deeper dive, visit the https://epam.github.io/edp-ddm-architecture/en/platform/1.9.6/arch/architecture/platform-technologies.html[Platform technology stack] page.
...
https://epam.github.io/edp-ddm-architecture/en/platform/1.9.6/platform-develop/coding-standards.html[Recommended software development and quality control practices].
...
* *Code quality*: A code quality control tool checks for common issues, ensuring the code aligns with established quality standards for the programming language. A versatile tool for this is SonarQube.
...
* *Docker building*: A Docker image is created from the Dockerfile, which is then stored in the current Registry with the corresponding VCS tag for further deployment and testing.
...
* *Git tag*: This step involves adding a git tag to the main branch in the VCS repository, corresponding to the artifact's version in the Registry. The tag might include the build number and match the artifact's name.

* *Docker image push*: Pushing the built docker image to storage in the Registry for further deployment on the selected environments.
...
** For _Build_ and _Deploy_ pipelines, it is recommended to use the https://defectdojo-portal.diia.digital/dashboard[DefectDojo] platform for security and vulnerability management for subsequent analysis and handling by development and security engineers.
...
* *QA Testing*: Manual quality assurance of the code according to predefined functionality use scenarios. Typically, it requires manual approval from a QA engineer.

* *Promote*: Promotes docker images to the Registry.
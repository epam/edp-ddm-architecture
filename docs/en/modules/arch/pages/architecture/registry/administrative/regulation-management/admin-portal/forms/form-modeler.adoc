...
=== Viewinf the _JSON_ code for representing UI form schemes
//=== Перегляд _JSON_-коду представлення схем UI-форм

This is consideren in the separate task -- xref:architecture-workspace/research/admin-portal/json-editor-tech-evaluation.adoc[Analysis of JSON editors for compliance].

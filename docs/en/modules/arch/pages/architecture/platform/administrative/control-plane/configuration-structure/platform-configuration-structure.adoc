...
|❌
|✅
//|Параметри кореляції версії реєстру та версії адмін-консолі.
|Registry-version-to-admin-console-version mapping parameters.
...
|A list of users with the Platform administrator role (`cp-cluster-mgmt-admin`).
...
|Platform deployment mode. Determines whether `external-integration-mocks` are present or not.
...
The `whiteListIP` group contains access parameters for the administrative services routes.
...
|Platform administrative services route access parameters.
...
The `consoleVersions` group contains registry-version-to-admin-console-version mapping parameters.
...
The `digitalSignature` group contains a list of the Platform's _Digital signature service_ settings.
...
|_Digital signature service_ key settings.
...
|_Digital signature service_ environment settings.
...
|Backup configuration of the Platform components.
...
|Backup configuration of the _Platform and registries management subsystem's_ components.
...
|Backup configuration of the Platform artifacts repository in the _Platform and registries deployment and configuration subsystem_.
...
|Backup configuration of the _Event monitoring and notification subsystem's_ components.
...
|Backup configuration of the _Users and roles management subsystem's_ components.
...
|A subdomain of the base domain of the OKD cluster for routing traffic to Platform applications and registries -- for example, `apps.openshift.example.com`.

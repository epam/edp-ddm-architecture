...
|❌
|❌
|Determines if external IDP integration is enabled.

|`<<externalidp-spec,externalIDP>>`
|object
|❌
|❌
|Settings for external IDP integration if enabled.
...
[[externalidp-spec]]
[cols="20%,15%,7%,7%,60%",options="header",caption=]
.portals.<portal_name>.externalIDP
|===
|Name |Type |Default value |Required |Description
...
|❌
|✅
|The Authorization URL of the external ID

|`backchannelSupported`
|bool
|false
|✅
|Does the external IDP support a backchannel logout

|`issuer`
|string
|❌
|❌
|The issuer identifier the issuer of the response

|`logoutUrl`
|string
|❌
|❌
|End session endpoint to logout user from external IDP.
...
|❌
|✅
|The path to the client id and client secret in the Hashicorp Vault _Secrets and encryption management service_.

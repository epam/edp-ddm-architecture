...
WARNING: Під час пошуку за текстовими полями (text, varchar, char) значення приводяться до нижнього регістру. Враховуйте це при створенні індексів за допомогою тегу `createIndex` або `SQL-запитів`.
...
----
.SQL-скрипт (_пошуковий запит за полями текстових типів_)
[source,sql]
----
SELECT name, type FROM table_one
WHERE lower(cast(name as varchar)) = lower('значення_пошуку')
----
...
WHERE legal_entity_name ILIKE 'значення_пошуку%'
...
----
SELECT name, code FROM table_two
WHERE code ILIKE '%значення_пошуку%'
----
...
----
SELECT first_name, last_name FROM users
WHERE lower(first_name) in (lower('Viktor'), lower('Dmytro'))
----
====
...
----
SELECT first_name, last_name FROM users
WHERE user_age IN (25, 30, 35) AND lower(cast(location as varchar)) = lower('Kyiv') AND is_active = true
----
====
...
WHERE lower(name) IN (lower('O''Reilly), lower(Jr.'), lower('Smith, Sr.'))
AND lower(location( IN (lower('New York, NY'), lower('Los Angeles, CA'))
----
====

...
+
TIP: Детальніше про атрибут `fetchType` та сценарії його застосування див. у розділі xref:#nested-structures[].
...
[source,bash]
----
GET https://.../subject-name-starts-with?legal_entity_name=значення_пошуку
----
+
...
* `legal_entity_name=Corp` -- параметр запита, який вказує на пошук юридичних осіб, чиї назви починаються з `Corp`.
...
----
+
TIP: Детальніше про атрибут ліміт ви можете дізнатися у секції xref:limit-attribute-values[].
+
...
* `user_age` -- вік користувача, який може бути використаний для множинного пошуку за допомогою оператора `in`.

TIP: Детальніше про атрибут `returning` ви можете дізнатися у секції xref:#returning-attribute-values[].
--
...
WHERE user_age IN (значення_пошуку)
----
+
--
У цьому запиті:

* Вибрані імена (`first_name`) та прізвища (`last_name`) користувачів з таблиці `users_table`.
* Пошук виконується за віком (`user_age`), який повинен бути одним зі значень, вказаних у множині `значення_пошуку`.
--
...
[source,bash]
----
GET https://.../find-in-age?user_age=значення_пошуку
----
+
...
* `https://api.example.com/find-in-age` -- це базовий URL ресурсу та ендпоінт, де відбувається пошук.
* `user_age=25,30,35` -- параметр запита, який вказує на пошук користувачів з віком 25, 30 або 35 років.

`*notIn*`::

повертає значення, що не дорівнюють жодному із заданих значень у масиві. Він є протилежним до значення `in` атрибута `searchType`.
+
.XML-схема
[source, xml]
----
...
* `first_name` -- ім'я користувача, яке повертається як результат.
* `last_name` -- прізвище користувача, яке також повертається як результат.
* `user_age` -- вік користувача, який може бути використаний для множинного пошуку, але з виключенням значень, вказаних в операторі `notIn`.
...
----
SELECT first_name, last_name FROM users
WHERE user_age NOT IN (значення_пошуку)
----
+
--
У цьому запиті:
...
* Пошук виконується за віком (`user_age`), який НЕ повинен бути одним зі значень, вказаних у множині `значення_пошуку`.
--
...
[source,bash]
----
GET https://.../find-not-in-age?user_age=значення_пошуку
----
+
Цей HTTP-запит використовує метод GET для запита до сервера з метою отримання результатів, які _НЕ_ відповідають критерію пошуку "знаходяться у списку" для поля `user_age`.
...
* `https://api.example.com/find-not-in-age` -- це базовий URL ресурсу та ендпоінт, де відбувається пошук.
* `user_age=25,30,35` -- параметр запита, який вказує на пошук користувачів, які НЕ мають вік 25, 30 або 35 років.

`*between*` ::

повертає значення, що належать до заданого діапазону значень (включно "від" та "до").
+
.XML-схема
[source, xml]
----
...
* `user_age_from=20&user_age_to=30` -- параметри запита, які вказують на пошук користувачів з віком від 20 до 30 років (включно).

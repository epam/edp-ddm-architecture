...
[source, XML]
----
<createTable tableName="table_1" ext:historyFlag="true">
...
[NOTE]
====
Для `primaryKey` сутності наполегливо рекомендуємо використовувати тип даних `UUID`, як показано на прикладі: `type="UUID"`.
====
...
TIP: Ознайомтеся також зі сценаріями створення таблиць: xref:registry-develop:data-modeling/data/physical-model/liquibase-changes-management-sys-ext.adoc#create-table-hst[Схема моделювання таблиць та функція підтримки історичності].
...
* Для полів типу дати (TIMESTAMPTZ, DATE), що використовуються для пошуку, слід явно вказувати тип, наприклад `type="TIMESTAMPTZ"`, інакше під час створення індексу може виникнути помилка.
...
* Поле головної таблиці, що містить посилання на підпорядковану таблицю має **обов'язково** посилатися на первинні ключі та бути масивом.
====

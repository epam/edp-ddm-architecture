...
. Перейдіть до розділу швидких посилань реєстру в інтерфейсі `Control Plane` та оберіть _Сервісу управління користувачами та ролями (Keycloak)_.
+
...
NOTE: Результатом створення користувача -- є вдалий вхід у _Кабінет користувача_.
...
NOTE: Для успішної автентифікації у _Кабінеті користувача_, користувач повинен існувати у `Keycloak`.
+

...
. Переконайтеся, що процес *`Restore-registry-admin-test`* в Jenkins завершився успішно. Для цього:

.. Відкрийте адміністративну панель *Control Plane*, перейдіть до швидких посилань реєстру та у розділі _Адміністративна зона Платформи_ оберіть _Сервіс розгортання конфігурації (*Jenkins*)_.
+
image:registry-develop:registry-admin-study/task-backup-restore/06-02-jenkins.png[]

.. Оберіть папку із потрібним реєстром та перейдіть до пайплайну *Restore-registry-`<registry-name>`*, де `<registry-name>` -- назва реєстру.
+

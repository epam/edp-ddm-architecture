= Завдання. Логування подій (Kibana)
...
Шаблон індексу -- це шаблон або правило, яке `Kibana` використовує для визначення індексів `Elasticsearch` (сховищ даних), які ви хочете проаналізувати. Шаблони індексів допомагають `Kibana` зрозуміти, які конкретні індекси в `Elasticsearch` слід використовувати під час візуалізації та дослідження даних.

Коли ми створюємо шаблон індексу, Kibana аналізує дані, що зберігаються у вибраних нами індексах, і отримує такі метадані, як імена полÍв (`fields`) і відповідні їм типи даних. Ця інформація є важливою для створення правильних візуалізацій і для фільтрації, агрегування та пошуку певних даних.

== Процес виконання завдання

=== Створення `Index Pattern` для реєстру

Користуючись п. 1.1-1.7  xref:registry-develop:registry-admin/openshift-logging/kibana.adoc[інструкції]  створіть індекс-паттерн для вашого реєстру.

* У Step1 використайте шаблон: app-<registry-name>-*
+
image:registry-develop:registry-admin-study/task-event-logging-kibana/01-index-pattern-step-1.png[]

* У Step2 встановіть 'timestamp' для фільтру по часу.
+
image:registry-develop:registry-admin-study/task-event-logging-kibana/02-index-pattern-step-2.png[]

=== Застосування фільтрів

. Перейдіть до вкладки `Discover` та змініть time range на останні 30 днів:
+
image:registry-develop:registry-admin-study/task-event-logging-kibana/03-time-range.png[]

. Оберіть новостворений індекс-паттерн app-<registry-name>-*  та створіть комбінований фільтр:
...
[NOTE]
В результаті комбінований фільтр покаже лише помилки у конкретній поді.
...
[NOTE]
В результаті відображатиметься загальна кількість записів за вказаний проміжок часу.

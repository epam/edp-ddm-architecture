...
[CAUTION]
====
Бізнес-процес *Створення лабораторії* очікує, що в системі вже є записи про суб'єктів (юридичних осіб або ФОП).

Такі записи додаються під час створення моделі даних у Завданні 1 через завантаження CSV-файлу `dict_subject.csv`.

У разі використання значення, якого немає в таблиці `subject`, сервісне завдання на початку процесу не зможе знайти суб'єкта та завершиться з помилкою: *"Суб'єкт не знайдено"*.
====
...
*_Змоделюйте сервісне завдання (Service Task)_*, яке перевірятиме, чи існує суб'єкт (юридична особа або ФОП), який буде пов'язаний із лабораторією, за кодом ЄДРПОУ/РНОКПП та типом суб'єкта перед додаванням нового запису лабораторії в систему.

IMPORTANT: Це завдання **не виконує пошук безпосередньо в таблиці `laboratory`**, а перевіряє наявність запису про суб'єкт у таблиці `subject`. *Лише після успішної перевірки суб'єкта* можна переходити до створення та прив'язки нового запису лабораторії.
...
[NOTE]
====
Назва ресурсу -- це назва API-ендпоінту у Фабриці даних,
який пов'язаний зі змодельованим попередньо Search Condition.
Завдяки цьому ендпоінту буде відбуватися пошук необхідних даних у БД.

Зверніть увагу, що при моделюванні бізнес-процесу необхідно
використовувати назву Search Condition через дефіс (`-`)
замість нижнього підкреслювання (`_`), як це визначено у `.xml`-файлі з описом моделі даних, що складаються з 2-х і більше слів.

Наприклад:

* Назва ресурсу (ендпоінту) у бізнес-процесі: `laboratory-equal-edrpou-name-count`.
* Назва search condition в `.xml`-файлі моделі даних, в якому він описаний: `laboratory_equal_edrpou_name_count`.
====
...
Змінна `dataPayload` сформована у попередньому скрипт-завданні
xref:#create-script-task-prepare-data-record-transient-var[].
...
[CAUTION]
====
Щоб налаштувати обробку помилки під час збереження даних до Фабрики даних у BPMN:

. Додайте подію *Intermediate/Boundary event* до цього сервісного завдання.
. Налаштуйте подію як *Error boundary event* (натисніть на значок ключа та виберіть *Error boundary event*).
. Додайте XOR-шлюз (*Exclusive Gateway*) перед завданням *Підписати дані про лабораторію*.
. Зв'яжіть Error boundary event із XOR-шлюзом стрілкою.
+
image:registry-develop:study-project/task-3/t5.png[]

У разі помилки при збереженні даних користувач автоматично потрапить на шлях повторного підписання даних КЕП (*Підписати дані про лабораторію*).
====
...
API ::
Ім'я поля таблиці у структурі даних, яка використовується для обміну інформацією між формою та API. Це поле:

- передається у структурі даних при відправленні форми;
- використовується для передзаповнення форми при її відкритті;
- відповідає назві поля у базі даних (_якщо значення зберігається у базі_);
- відображається у вкладці *Запит* в інтерфейсі розробки форм.

* *Property Name:* `subjectType`

[NOTE]
====
Імена полів у структурі даних використовуються у форматі *camelCase*: перше слово з малої літери, наступні слова з великої, без пробілів.
Наприклад: `propertyName`.

У базі даних ці поля автоматично трансформуються у формат *snake_case*: `property_name`.
====
...
[NOTE]
====
На етапі створення UI-форми обов'язково налаштуйте валідаційні правила для всіх полів, значення яких користувач має заповнювати вручну.
====
+
...
* `/officer` -- вказує, що запит до довідника буде виконано із Кабінету надавача послуг.
Такий вказівник встановлюється лише для надавача послуг (`officer`).
Для отримувача послуг (`citizen`) шлях починається без префікса, безпосередньо з `/api`.
...
Наприклад: налаштування автоматичного закриття бізнес-процесу, незавершеного користувачем, можливість повернутися на попередній етап або відразу до головного меню, налаштування сповіщень користувачу у його персональний кабінет тощо.

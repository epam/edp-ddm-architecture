...
[TIP]
====
Детальніше ознайомитися з процесом налаштування бізнес-ключів можна на сторінці xref:registry-develop:bp-modeling/bp/modeling-facilitation/bp-business-keys.adoc[].
====
...
TIP: В результаті маємо змодельований бізнес-процес з інтеграцією Фабрики даних.
...
. Заповніть поле `Службова назва форми` значенням `add-lab-sign-lab-data` (відповідає значенню поля `Form key` тієї ж xref:#create-user-task-lab-data-signing[користувацької задачі]);
...
На основі наданого контексту, процедура перевірки виконання завдання розробки регламенту для розробників може бути оформлена таким чином:

...
Однак ви можете одразу клонувати репозиторій вашого проєкту із регламентом, використовуючи покрокову інструкцію xref:registry-admin/regulations-deploy/registry-admin-deploy-regulation.adoc[Операції з регламентом в Gerrit] або повнішу інструкцію xref:registry-admin/cp-deploy-consent-data.adoc[Розгортання демо-реєстру із референтними прикладами].
У регламенті всі `.xml`-файли для моделювання даних уже створені, зокрема:
...
+
У цьому блоці коду створюється кастомний тип `type_subject_type` як enum-список, що використовується для визначення типу суб'єкта.
ChangeSet включає метадані, такі як автор і унікальний ідентифікатор, які відстежують цю зміну.
Значення enum включають переклади для кожного значення, що дозволяє використовувати їх у локалізованих інтерфейсах.
+
[source,xml]
----
<?xml version="1.0" encoding="UTF-8"?>
...
(наприклад, `"UUID"`, `"TEXT"`)
|`defaultValueComputed` _(для первинного ключа)_
| `"uuid_generate_v4()"`
|=================
+
В результаті розширюємо нашу структуру наступним чином:
...
* Використовуйте _ЛИШЕ_ тип *UUID* для первинного ключа таблиць і функцію uuid_generate_v4() як значення за замовчуванням. Ця функція згенерує  випадкове числове значення (_див. https://www.uuidgenerator.net/version4_).
====
...
Після створення структури бази даних потрібно завантажити початкові (довідникові) дані. Їх _завантаження можливе до початку роботи реєстру_.
Це важливо для коректної роботи бізнес-процесів, які використовують ці довідники, наприклад, *Створення лабораторії*.

Дані завантажуються з CSV-файлів до відповідних таблиць за допомогою функції `p_load_table_from_csv()` у Liquibase. Виклик функції можливий через відповідний Liquibase-тег – *`<sql>`*.

=== Як це працює

. CSV-файли розміщуються у папці `_data-model/data-load/`.
. Liquibase копіює ці файли на сервер у каталог `/tmp/data-load/`.
. У файлі `link:{attachmentsdir}/study-project/task-2/xml-temp/populateDictionaries.xml[populateDictionaries.xml]` додаються виклики функції `p_load_table_from_csv()` для кожного CSV-файлу.
. Цей файл підключається у `main-liquibase.xml`.
...
CALL p_load_table_from_csv(
  'subject',
  '${dataLoadPath}dict_subject.csv',
  array['subject_code','subject_type','subject_status','subject_name']
);
----

[TIP]
====
Задайте шлях до CSV-файлів через змінну:
...
=== Де розміщувати файли

Розмістіть усі CSV-файли у папці `_data-model/data-load/`.
Після того, як модель буде завантажена у Gerrit, система автоматично скопіює їх у `/tmp/data-load/` на сервері бази даних.

=== Список CSV-файлів для завантаження

[options="header"]
|===
|Довідник                |Файл з даними
|КОАТУУ (опційно)        |`link:{attachmentsdir}/study-project/task-2/csv-dict/dict_koatuu_workshop.csv[dict_koatuu_workshop.csv]`
|Тип власності           |`link:{attachmentsdir}/study-project/task-2/csv-dict/dict_formy_vlasnosti.csv[dict_formy_vlasnosti.csv]`
|Статус співробітника    |`link:{attachmentsdir}/study-project/task-2/csv-dict/dict_status_spivrobitnyka.csv[dict_status_spivrobitnyka.csv]`
|Суб'єкти (ФОП / юрособи)|`link:{attachmentsdir}/study-project/task-2/csv-dict/dict_subject.csv[dict_subject.csv]`
|===


* https://www.youtube.com/watch?v=Q16a8eL3OKA&list=PL4ZHbn5p85eWowuRWY_5UWiO4RdLF_ykh&index=15[Відео по темі: Завантаження даних за допомогою Liquibase скриптів регламенту]


== Застосування розробленої моделі до бази даних

Платформа використовує *`main-liquibase.xml`* як основний файл для розгортання моделі даних реєстру.

NOTE: Всі changeSet-и, що будуть включені до файлу `main-liquibase.xml`, застосуються в базі даних.

Для включення changeSet-у із файлів, створених протягом попередніх кроків, використовується тег `*<include>*` з атрибутом `*file*`, що вказує шлях до XML-файлу.
Поточною директорією для Liquibase є коренева папка Gerrit-репозиторію -- тому шлях до файлів має наступний вигляд: *_data-model/+++*.xml+++_*.

Щоб застосувати створену модель, виконайте наступні кроки: ::
...
[TIP]
====
Усього маємо отримати *9 файлів* для розгортання моделі даних та первинного наповнення БД:
...
4 файли CSV із довідниками для первинного наповнення: ::
...
** `link:{attachmentsdir}/study-project/task-2/csv-dict/dict_subject.csv[dict_subject.csv]`
...
Після виконання всіх кроків і успішного проходження збірки регламенту, ваш реєстр міститиме змодельовану структуру бази даних, заповнену необхідними довідниками для подальшої роботи бізнес-процесів.
...
* https://www.youtube.com/@%D0%9F%D0%BB%D0%B0%D1%82%D1%84%D0%BE%D1%80%D0%BC%D0%B0%D1%80%D0%B5%D1%94%D1%81%D1%82%D1%80%D1%96%D0%B2/playlists[Навчальні відео по роботі Платформи]
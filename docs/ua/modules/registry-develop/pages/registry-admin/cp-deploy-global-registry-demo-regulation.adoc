...
[IMPORTANT]
====
Для проходження данної інструкції адміністратор платформи повинен бути доданий як адміністратор реєстру під час його створення у розділі _ADMINISTRATORS_.



При розгортанні реєстру в регіоні _global_ такі розділи як _KEY DATA_ та _DATA FOR SIGNATURE VERIFICATION_ не мають можливості додавання специфічних для регіону ключів та сертифікатів

image:registry-admin/cp-deploy-global-registry-demo-regulation/cp-deploy-global-registry-demo-regulation-1.png[]

====

. Відкрийте консоль адміністратора *Control Plane* > перейдіть на пункт меню +++<b style="font-weight: 700">PLATFORM MANAGEMENT > далі вкладка QUICK LINKS<b>+++ та перейдіть за посиланням до центрального компонента *Gerrit*.
+
image::admin:registry-management/control-plane-platform-management-quick-links.png[]
...
. Відкрийте консоль адміністратора *Control Plane* > перейдіть на пункт меню REGISTRIES +++<b style="font-weight: 700"> > далі вкладка QUICK LINKS<b>+++ та перейдіть за посиланням до реєстрового компонента *Gerrit*.
+
image:admin:registry-management/quick-links/quick-links-2.png[]
+
. Перейдіть до налаштувань облікового запису Gerrit та знайдіть розділ *HTTP Credentials*.
+
image:registry-admin/cp-deploy-consent-data/cp-deploy-consent-data-2.png[]
...
=====
====
Кожна інтеграція додається окремо, та буде застосована після підтвердження Change Request:
+
...
+
TIP: Посилання до Кабінету адміністратора регламентів можна знайти у Control Plane > перейшвши на пунтк меню REGISTERS > Перехід до реєстру(demo)> далі вкладка QUICK LINKS > Registry modelling web interface (Administrative portal)
+
...
=====
====
У регламенті демо-реєстру представлені процеси, які залежать від мок-інтеграцій з зовнішніми системами. Наприклад, у файлі _mock-integrations/population-registry.json_ представлені мокові дані користувачів, які можна використовувати в процесі _Create driving licence_.
====
[%collapsible]

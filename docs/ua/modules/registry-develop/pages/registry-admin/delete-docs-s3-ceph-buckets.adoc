...
. Завантажте утиліту xref:attachment$registry-admin/delete-docs-s3/storage-cleanup-cli-1.4.0-20231218.110538-1.jar[_storage-cleanup-cli.jar_].

. Запустіть утиліту наступним чином:
+
[source,bash]
----
...
* `--removal-batch-size=<arg>` -- batch-розмір для видалення (_за замовчуванням_ -- `1000`).

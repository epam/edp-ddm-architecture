...
* `/api/gateway/business-process/{bp-definition-id}` -- для детального розмежування та керування доступом до конкретних бізнес-процесів. Ingress повинні бути налаштовані з унікальними шляхами, що відповідають ідентифікаторам бізнес-процесів. Це забезпечує точне спрямування запитів до відповідних компонентів системи.
+
TIP: `{bp-definition-id}` вказує на ідентифікатор бізнес-процесу у BPMS.

[NOTE]
====
Підготовка плагіну і секретів для бізнес-процесів ::
Перед інтеграцією плагіну для управління доступом до API бізнес-процесів, вам потрібно створити відповідний `KongPlugin` та асоційований з ним секрет, що містить конфігурацію для плагіну (_див. детальніше -- xref:#create-kong-plugin-secret[]_). Цей крок є критичним для забезпечення можливості застосування плагіну через анотації в ingress (`konghq.com/plugins`). Процедура створення плагіну та секрету для бізнес-процесів схожа на описану для файлових ендпоінтів, але адаптована під специфіку управління бізнес-процесами через API.
====

...
Нижче наведено кроки для налаштування рейт-лімітів для трьох ключових сценаріїв: пошукові умови, файлові ендпоінти та бізнес-процеси.
...
*Rate limiting `KongPlugin`* використовується на рівні Kong API Gateway, щоб застосувати правила `rate-limiting` для певного URL. Підключається до `Ingress`. Для правильної роботи має містити посилання на ресурс `Secret`, який містить інформацію про кількість допустимих запитів за одиницю часу.
...
+
*Rate limiting `Secret`*: використовується у rate-limiting `KongPlugin`. Містить інформацію про кількість допустимих запитів за одиницю часу (_можливі одиниці часу:_ `second`, `minute`, `hour`, `day`, `month`, `year`), а також адресу Redis, де має зберігатися лічильник.
...
*Rate limiting `Service`* використовується як `host` при конфігурації адреси, де зберігається лічильник у rate-limiting Secret. Виступає як проксі для запитів до Redis.
...
+
*Backend `Ingress`* визначає правила доступу до конкретного URL всередині системи. У ньому потрібно вказати назву rate-limiting Kong Plugin, а також повну інформацію про URL, для якого конфігурується рейт-ліміт (`host`, `path`, HTTP-метод). Якщо рейт ліміт не досягнуто, запит перенаправляється на backend `Service`.
+
...
*Backend `Service`* визначає набір подів, які обробляють запит, що пройшов з `Ingress`. На цьому рівні налаштування рейт лімітів не відбуваються.
...
* `/api/gateway/business-process/` -- для розмежування доступу до усіх бізнес-процесів;
...
spec:
  ...
  rules:
    - host: external-service-api-<registry-name>-main.apps.<cluster-name>.<dns-wildcard>
      http:
        paths:
          - path: /api/gateway/business-process/{bp-definition-id}
            pathType: ImplementationSpecific
            backend:
              service:

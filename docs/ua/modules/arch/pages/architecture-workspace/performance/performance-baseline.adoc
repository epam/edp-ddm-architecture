...
| Час завантаження форми-вводу для кінцевого користувача
| avg(get form duration)
| avg([portal][get-form-by-key] duration)
| ms

| Загальна кількість виконаних Бізнес процесів
| sum(complete tasks) - (sum(start tasks) - sum(complete tasks))
| sum([portal][bp:${bp-name}][task:${task-name}][complete/sign-form]) - (sum([portal][bp:${bp-name}][task:${task-name}][start-with-form]) - sum([portal][bp:${bp-name}][task:${task-name}][complete/sign-form]))
| amount

| Пропускна здібність на запуск нових бізнес процесів
| sum(start with form tps)
| sum(([portal][bp:${bp-name}][task:${task-name}][start-with-form]) tps)
| req/sec

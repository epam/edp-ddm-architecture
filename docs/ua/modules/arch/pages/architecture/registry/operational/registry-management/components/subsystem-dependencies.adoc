...
[registry-kafka-api] -up-> [operational pool]
[registry-kafka-api] -> [registry-rest-api]
[registry-kafka-api] -down-> [kafka-cluster]
...
note bottom of [registry-kafka-api]
1. To Start: init container (postgres, registry-rest-api),
readiness probe (to Postgres, to Kafka, to DSO)
end note

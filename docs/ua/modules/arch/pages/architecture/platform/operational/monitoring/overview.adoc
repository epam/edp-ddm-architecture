...
|etcd
.16+|xref:arch:architecture/container-platform/container-platform.adoc[Платформа оркестрації контейнерів]
...
|
|cluster-total
|Загальні метрики використання реєстрами ресурсів платформи оркестрації контейнерів OKD.

...
. (*ОПТИМАЛЬНИЙ варіант*, не потребує створення route) Зробити port-forward до сервісу bp-webservice-gateway за допомогою oc client та наступної link:https://diia-engine.github.io/diia-engine-documentation/ua/platform/1.9.8/registry-develop/registry-admin/remote_connection.html[інструкції]
+

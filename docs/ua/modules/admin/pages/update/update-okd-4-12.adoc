...
[CAUTION]
====
Після видалення машини важливо зачекати *2-3 хвилини* перед тим, як переходити до наступного кроку: xref:#apply-new-machine-config[Застосування файлу конфігурації нової машини]. Недотримання цієї паузи може призвести до непередбачених наслідків, таких як проблеми з мережею на новій ноді тощо. Якщо виникають проблеми із запуском нової ноди, потрібно повторно виконати процес видалення, починаючи з кроку xref:#shut-down-master-node[Вимкнення мастер-ноди].
====
+
...
.. Після підключення до проблемної ноди через *SSH*, введіть наступну команду у терміналі цієї ноди:
+
[source,bash]
----

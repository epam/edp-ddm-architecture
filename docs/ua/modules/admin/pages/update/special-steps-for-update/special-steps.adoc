...
Запустіть пайплайн `platform-deploy` в Jenkins CICD2 з опціями оновлення та необхідною версією збірки, а саме `1.9.7.42`.
...
. У автоматично відкритому вікні пошуку файлів вкажіть наступний шлях:
...
. У вікні редагування знайдіть блок для репозиторію _service-mesh_ та змініть поля `version` та `branch` на назву гілки, яку було створено одному з попередніх кроків. У нашому випадку назва гілки -- `1.9.7.7-jaeger-update`.
+
...
+
Також перевірте наявність роутів. Це можна зробити у розділі *Networking* > *Routes* проєкту `istio-system`. Має існувати роут `jaeger`.
+
...
----
+
У цьому випадку наступні параметри за замовчуванням, що не стосуються AWS-провайдера, було видалено:
+
[source,yaml]
----
vSphereInstanceCPUCount: 8
...
*Project*: *`control-plane`* > *Codebases* > *Codebase details*
...
Після цього натисніть *`Reply`* > *`Code-Review +2`*, *`Verify +1`* > *`Submit`* та запустіть пайплайн *MASTER-Build-cluster-mgmt*. Дочекайтеся завершення збірки коду.

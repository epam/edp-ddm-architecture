...
Права доступу на адміністрування та моделювання регламенту в xref:registry-develop:registry-admin/admin-portal/overview.adoc[адміністративному порталі] надає адміністратор Платформи у реалмі `openshift` сервісу Keycloak.
...
////
RELEVANT, OLD APPROACH INSTRUCTION, NOT RECOMMENDED
. Перейдіть до *Projects* > *user-management*.
+

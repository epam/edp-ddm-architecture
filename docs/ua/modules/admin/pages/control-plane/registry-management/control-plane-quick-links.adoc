...
|image:registry-management/quick-links/logos/tekton-logo.svg[width=50,height=auto]

|*Tekton*: Сервіс розгортання регламенту

|Програмний комплекс, що забезпечує автоматизацію в життєвому циклі розгортання регламенту реєстру.

a|
----
https://admin-tools-<registry-name>-main.<dns-wildcard>/tekton
----
...
https://admin-tools-platform-demo-main.example.com/tekton

|
include::platform:ROOT:partial$templates/links/registry/administrative/tekton-dashboard.adoc[]
...
a|Сховище артефактів, компонентів реєстру та їх залежностей, з яких складається кожна окрема підсистема реєстру. Збереження згенерованих в реєстрі артефактів.

NOTE: Сервіс доступний лише за умови використання реєстром окремого сховища. Параметр можна вказати при
xref:admin:control-plane/registry-management/control-plane-create-registry.adoc[розгортанні]
або
xref:admin:control-plane/registry-management/control-plane-edit-registry.adoc[редагуванні]
реєстру.

a|
----
https://nexus-control-plane-<registry-name>-main.<dns-wildcard>/nexus
----

* `<registry-name>` -- назва реєстру;
* `-main` -- системна константа;
* `<dns-wildcard>` -- визначає домен та піддомени середовища;
* `/nexus` -- ендпоінт доступу до сервісу.

*Наприклад:*

https://nexus-control-plane-platform-demo-main.example.com/nexus

|
include::platform:ROOT:partial$templates/links/registry/administrative/nexus.adoc[]

|image:registry-management/quick-links/logos/rest-api-logo.svg[width=50,height=auto]

|*OpenAPI/Swagger*: API-документація сервісу управління даними реєстру

|Вебінтерфейс для перегляду API-документації Підсистеми управління даними реєстру з ціллю подальшого використання при побудові взаємодії через типові розширення-конектори у бізнес-процесах.

a|
----
https://registry-rest-api-<registry-name>.<dns-wildcard>/swagger-ui
----
...
https://registry-rest-api-platform-demo.example.com/swagger-ui/index.html
...
|*OpenAPI/Swagger*: API документація сервісу управління даними реєстру *для публічних даних*

|Вебінтерфейс для перегляду API-документації Підсистеми управління даними реєстру з ціллю подальшого використання при побудові взаємодії через типові розширення-конектори у бізнес-процесах.

a|
----
https://platform-gateway.<dns-wildcard>/api/public/data-factory/openapi
----
...
https://platform-gateway.example.com/api/public/data-factory/openapi

|
include::platform:ROOT:partial$templates/links/registry/administrative/openapi-public-data.adoc[]
...
|image:registry-management/quick-links/logos/kafka-logo.svg[width=50,height=auto]

|*Kafka UI*: Вебінтерфейс моніторингу черг повідомлень

|Вебінтерфейс для перегляду черг повідомлень і повідомлень в них.

a|
----
https://admin-tools-<registry-name>-main.<dns-wildcard>/kafka-ui
----
...
https://admin-tools-platform-demo-main.example.com/kafka-ui

|
include::platform:ROOT:partial$templates/links/registry/administrative/kafka.adoc[]

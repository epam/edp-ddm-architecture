...
* DynamoDB table - використовується для збереження інформації про блокування стану терраформу;
* NAT Gateway - використовується для забезпечення приватного серверу доступом до Інтернету;
* Bastion - використовується як проміжний сервер для забезпечення безпечного та обмеженого доступу до серверу у приватній мережі. Надалі, через цей bastion буде прокинуто SSH-тунель до deployer-node;
* Deployer-node -  сервер у приватній мережі, через який буде відбуватися інсталяція кластеру та платформи.
...
. Для подальших кроків необхідно отримати доступ до файлів з іншого, зовнішнього облікового запису AWS. Для цього необхідно звернутися до команди підтримки, які повинні додати ID Вашого облікового запису AWS в Trust relationships для ролі `CustomCrossAccountRole` в їх обліковому записі AWS.
...
* `files/user_data.sh.tpl` - шаблон скрипта, який буде виконуватися при створенні або оновленні EC2 інстансу в середовищі AWS. Цей скрипт зробить наступне для deployer-node:
** встановить Docker;
** встановить Unzip;

...
* *DynamoDB* -- таблиця в DynamoDB, яка буде використовуватися для збереження інформації про блокування стану Terraform.
* *NAT Gateway* -- використовується для забезпечення приватного сервера доступом до Інтернету.
* *Bastion* -- використовується як проміжний сервер для забезпечення безпечного та обмеженого доступу до сервера у приватній мережі. Надалі, через цей бастіон буде прокинуто SSH-тунель до initial-node.
* *Initial-node* -- сервер у приватній мережі, через який буде відбуватися інсталяція кластера та платформи.

Розгорнути ці ресурси можна за допомогою підготовленого Terraform-коду в наступних кроках.
...
* `security-groups.tf` - створюються Security Groups, які дозволяють SSH-з'єднання (TCP порт 22) для бастіону та initial-node.
* `ssh-key.tf` -- містить код для створення SSH приватного ключа та збереження ключа у файл та налаштування його прав доступу.
* `files/user_data.sh.tpl` -- шаблон скрипту, який буде виконуватися при створенні або оновленні EC2 інстансу в середовищі AWS. Цей скрипт зробить наступне для initial-node:
** встановить Docker;
** встановить Unzip;
